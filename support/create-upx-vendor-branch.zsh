...
[[ -d $tmpdir ]] || exit 1
cp -ai A* C* R* acc include src $tmpdir
...
rm */Makefile.* */*/Makefile.* include/ucl/ucl_asm.h
rm acc/acclib/[a-gi-z]*.ch acc/acclib/h[b-ln-z]*.ch
# 5) cleanup files (whitespace)
find . -type d -name '.git' -prune -o -type f -print0 | xargs -0r sed -i -e 's/[ \t\x0c]*$//'
